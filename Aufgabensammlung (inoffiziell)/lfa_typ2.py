...
#########################################
		...
		self.actionExit.setText(_translate("MainWindow", "Exit", None))
		self.actionE.setText(_translate("MainWindow", "Exit", None))
		#self.change_to_full_gk_name(ag_beschreibung)
	...
	def refresh_ddb(self):
		...
		beispieldaten = []
		###### Typ 1 ###
		#for root, dirs, files in os.walk(os.path.join(os.path.dirname(__file__),"Aufgabensammlung (inoffiziell)\Typ 1 Aufgaben")):
		###################
		# print(os.path.join('.','Typ 2 Aufgaben'))
		# return
		for root, dirs, files in os.walk(os.path.join('.','Typ 2 Aufgaben')):
			for all in files:
		...
		for root, dirs, files in os.walk(os.path.join(os.path.dirname(os.path.dirname(__file__)),"Aufgabensammlung (offiziell)\Typ 2 Aufgaben")):
			for all in files:
				while_cnt=0
				if all.endswith('.tex') or all.endswith('.ltx'):
					if not ('Gesamtdokument' in all) and not ('Teildokument' in all):
		...
		##### Typ 1 ###
		###log_file=os.path.join(os.path.dirname('__file__'),'Typ 1 Aufgaben','Teildokument','log_file')
		######
		log_file=os.path.join(os.path.dirname('__file__'),'Typ 2 Aufgaben','Teildokument','log_file')
	...
	def create_pdf(self):
		if sys.platform.startswith('linux'):
			...
			subprocess.Popen('cd "Typ 2 Aufgaben/Teildokument" & latex --synctex=-1 Teildokument.tex& dvips Teildokument.dvi & ps2pdf -dNOSAFER Teildokument.ps',shell=True).wait()
			subprocess.Popen('cd "Typ 2 Aufgaben/Teildokument" & Teildokument.pdf', shell=True).poll()
		## -interaction=nonstopmode -halt-on-error Don't stop when error occurs, while compiling
		os.unlink('Typ 2 Aufgaben/Teildokument/Teildokument.aux')
		os.unlink('Typ 2 Aufgaben/Teildokument/Teildokument.log')
		os.unlink('Typ 2 Aufgaben/Teildokument/Teildokument.dvi')
		os.unlink('Typ 2 Aufgaben/Teildokument/Teildokument.ps')
	...
	def PrepareTeXforPDF(self):
		...
		if not os.path.isfile(os.path.join('Typ 2 Aufgaben','Teildokument','log_file')):
			self.refresh_ddb()
		suchbegriffe = []
	...
	# 	if self.menu_searchtype.currentText()=='Alle Dateien ausgeben, die alle Suchkriterien enthalten':
		#######
		if self.menu_searchtype.currentText()=='Alle Dateien ausgeben, die ausschließlich diese Suchkriterien enthalten':
