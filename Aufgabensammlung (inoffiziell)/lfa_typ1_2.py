...
#########################################
	...
	def get_logfile(self):
		try:
			x='Typ %s Aufgaben'%self.label_aufgabentyp.text()[-1]
			log_file=os.path.join(os.path.dirname('__file__'),x,'Teildokument','log_file')
			self.label_update.setText(_translate("MainWindow", 'Letztes Update: ' + self.modification_date(log_file).strftime('%d.%m.%y - %H:%M'), None))
		except FileNotFoundError:
			self.label_update.setText(_translate("MainWindow", "Letztes Update: ---", None))
	...
	def refresh_ddb(self):
		...
		for root, dirs, files in os.walk(os.path.join('.',chosen_aufgabenformat)):
			for all in files:
	...
	def create_pdf(self):
		...
		if sys.platform.startswith('linux'):
			...
			subprocess.Popen('cd "Typ %s Aufgaben/Teildokument" & latex --synctex=-1 Teildokument.tex& dvips Teildokument.dvi & ps2pdf -dNOSAFER Teildokument.ps'%chosen_aufgabenformat,shell=True).wait()
			subprocess.Popen('cd "Typ %s Aufgaben/Teildokument" & Teildokument.pdf'%chosen_aufgabenformat, shell=True).poll()
		## -interaction=nonstopmode -halt-on-error Don't stop when error occurs, while compiling
		os.unlink('Typ %s Aufgaben/Teildokument/Teildokument.aux'%chosen_aufgabenformat)
		os.unlink('Typ %s Aufgaben/Teildokument/Teildokument.log'%chosen_aufgabenformat)
		os.unlink('Typ %s Aufgaben/Teildokument/Teildokument.dvi'%chosen_aufgabenformat)
	...
	def PrepareTeXforPDF(self):
		...
		filename_teildokument = os.path.join(os.path.dirname('__file__'),chosen_aufgabenformat,'Teildokument','Teildokument.tex')
		try:
	...
	# 	if self.menu_searchtype.currentText()=='Alle Dateien ausgeben, die alle Suchkriterien enthalten':
		#######
		if self.menu_searchtype.currentText()=='Alle Dateien ausgeben, die ausschließlich diese Suchkriterien enthalten' and chosen_aufgabenformat=='Typ 2 Aufgaben':
	...
	# 			dict_gesammeltedateien[all]=beispieldaten_dateipfdad[all]
		...
		if self.menu_searchtype.currentText()=='Alle Dateien ausgeben, die zumindest ein Suchkriterium enthalten' or chosen_aufgabenformat=='Typ 1 Aufgaben':
			gesammeltedateien=[]
			for all in suchbegriffe:
