...
newapp_path=os.path.join(os.path.dirname('__file__'),'_database','_config','update','LaTeX_File_Creator%s'%extension)
mainfile_path=os.path.join(os.path.dirname('__file__'),'LaTeX_File_Creator%s'%extension)
...
for i, item in enumerate(items):
    if i==50:
        if sys.platform.startswith('linux'):
            p=subprocess.Popen('cp "{0}" "LaTeX_File_Creator{1}"'.format(newapp_path, extension), stdout=subprocess.PIPE,shell=True)
        elif sys.platform.startswith('darwin'):
            p=subprocess.Popen('cp "{0}" "LaTeX_File_Creator{1}"'.format(newapp_path, extension), stdout=subprocess.PIPE,shell=True)
        else:
            p=subprocess.Popen('copy "{0}" "LaTeX_File_Creator{1}"'.format(newapp_path, extension), stdout=subprocess.PIPE,shell=True)
