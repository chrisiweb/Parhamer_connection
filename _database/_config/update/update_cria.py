...
updatefile_path=os.path.join(path_programm,'_database','_config','update','update_cria%s'%extension)
newapp_path=os.path.join(path_programm,'_database','_config','update','LaMA_Cria%s'%extension)
mainfile_path=os.path.join(path_programm,'LaMA_Cria%s'%extension)
...
for i, item in enumerate(items):
    if i==50:
        if sys.platform.startswith('linux'):
            p=subprocess.Popen('cp "{0}" "LaMA_Cria{1}"'.format(newapp_path, extension), stdout=subprocess.PIPE,shell=True)
        elif sys.platform.startswith('darwin'):
            p=subprocess.Popen('cp "{0}" "LaMA_Cria{1}"'.format(newapp_path, extension), stdout=subprocess.PIPE,shell=True)
        else:
            p=subprocess.Popen('copy "{0}" "LaMA_Cria{1}"'.format(newapp_path, extension), stdout=subprocess.PIPE,shell=True)
