...
Hauptplatz steigen sie in einen Wagen der Linie 3 um, in dem sie alle 18 Fahrgäste kontrollieren.%Aufgabentext

\begin{aufgabenstellung}
\item %Aufgabentext

\Subitem{Es soll die Wahrscheinlichkeit $p_1$ berechnet werden, dass die Kontrolleure mindestens eine Schwarzfahrerin/einen Schwarzfahrer ermitteln, aber erst in der Linie 3 auf diese Person treffen. Gib einen geeigneten Term an, mit dem diese Wahrscheinlichkeit $p_1$ ermittelt werden kann, und berechne diese.} %Unterpunkt1
\Subitem{Es sei $p_2$ die Wahrscheinlichkeit, bereits im Wagen der Linie 5 auf mindestens eine Schwarzfahrerin/einen Schwarzfahrer zu treffen. Begründe, warum $p_2$ größer als $p_1$ sein muss, ohne $p_2$ zu berechnen.} %Unterpunkt2
...
davon aus, dass im Durchschnitt die nicht erwischten Schwarzfahrer/innen jeweils entgangene Einnahmen eines Einzelfahrscheins von \EUR{2} verursachen.%Aufgabentext

\Subitem{Berechne den in einem Jahr durch die Schwarzfahrer/innen entstandenen finanziellen Verlust für die Grazer Linien.} %Unterpunkt1
\Subitem{Das Bußgeld müsste wesentlich erhöht werden, um eine Kostendeckung zu erreichen. Ermittle den neuen Betrag für ein kostendeckendes Bußgeld.} %Unterpunkt2

\item Die Anzahl der entdeckten Schwarzfahrer/innen nahm gegenüber 2009 um 300 ab und betrug 2010 nur mehr 36\,449. Man geht davon aus, dass durch verstärkte Kontrollen eine weitere Abnahme der Anzahl an Schwarzfahrerinnen/Schwarzfahrern erreicht werden kann. %Aufgabentext

\Subitem{Beschreibe diese Abnahme beginnend mit dem Jahr 2009 sowohl als lineares als auch als exponentielles Modell.

Gib jeweils einen Funktionsterm an, der die Anzahl $S$ der Schwarzfahrer/innen nach $t$ Jahren, ausgehend von dem Jahr 2009, beschreibt.} %Unterpunkt1
\Subitem{Berechne die Anzahl der Schwarzfahrer/innen nach 10 Jahren, also im Jahr 2019, mit beiden Modellen. Welche Schlussfolgerungen über die beiden Modelle ziehst du aus dem Ergebnis?} %Unterpunkt2
...
\Subitem{$p_1=0,95^{25}\cdot (1-0,95^{18})\approx 0,1672$} %Lösung von Unterpunkt1
\Subitem{Mögliche Argumentationen:
...
\Subitem{Ein Punkt für den korrekten Term sowie die entsprechende Wahrscheinlichkeit.} %Lösungschlüssel von Unterpunkt1
\Subitem{Ein Punkt für eine richtige Argumentation.} %Lösungschlüssel von Unterpunkt2

\item \subsection{Lösungserwartung:} 

\Subitem{Der zu erwartende Verlust wird wie folgt berechnet:
		...
		$=(-0,891\cdot 2+0,009\cdot 65)\cdot 5\,000\,000\approx -1,197\cdot 5\,000\,000\approx$ \EUR{-5.985-000}} %Lösung von Unterpunkt1
\Subitem{Soll der Verlust $V=0$ sein, dann gilt: $0=-0,891\cdot 2+0,009\cdot B$
		
		$\rightarrow B=$ \EUR{198}.
		
		Das Bußgeld $B$ müsste auf \EUR{198} erhöht werden.} %%Lösung von Unterpunkt2

\setcounter{subitemcounter}{0}
\subsection{Lösungsschlüssel:}
 
\Subitem{Ein Punkt für den korrekten Verlust.} %Lösungschlüssel von Unterpunkt1
\Subitem{Ein Punkt für das neue Bußgeld.} %Lösungschlüssel von Unterpunkt2

\item \subsection{Lösungserwartung:} 

\Subitem{lineare Abnahme: $S(t)=36\,749-300\cdot t$
		
		exponentielle Abnahme: $S(t)=36\,749\cdot (36\,449/36\,749)^t$} %Lösung von Unterpunkt1
\Subitem{Bei linearer Abnahme sind es nach 10 Jahren noch 33\,749, bei exponentieller Abnahme 33\,857 Personen. Der Unterschied ist gering und beide Modelle sind für diesen Zeitraum gleich gut.} %%Lösung von Unterpunkt2

\setcounter{subitemcounter}{0}
\subsection{Lösungsschlüssel:}
 
\Subitem{Ein Punkt für die beiden Funktionsgleichungen.} %Lösungschlüssel von Unterpunkt1
\Subitem{Ein Punkt für die korrekte Schlussfolgerung.} %Lösungschlüssel von Unterpunkt2
