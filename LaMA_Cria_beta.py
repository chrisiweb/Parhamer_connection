...
logo_path=os.path.join(path_programm,'_database','_config','icon','LaMa_icon_logo_blau.png')
...
widgets_search=['actionRefresh_Database','actionReset','actionLoad','actionSave','menuNeue_Schularbeit','menuNeue_Aufgabe','menuFeedback','menuHelp','label_update','groupBox_ausgew_gk','groupBox_af','groupBox_schulstufe',
'groupBox_titelsuche','groupBox_unterkapitel','cb_solution','btn_suche', 'combobox_searchtype'] #,'label_aufgabentyp','groupBox_gk','groupBox_klassen','groupBox_themen_klasse',

widgets_create=['actionReset','menuBild_einf_gen','menuSuche','menuNeue_Schularbeit','menuFeedback','menuHelp','groupBox_ausgew_gk_cr','groupBox_bilder','groupBox_grundkompetenzen_cr','groupBox_punkte','groupBox_aufgabenformat',
'groupBox_2','groupBox_beispieleingabe','groupBox_quelle','pushButton_save'] #,'groupBox_aufgabentyp','groupBox_klassen_cr'


widgets_sage=['actionRefresh_Database','menuSuche','menuNeue_Aufgabe','menuFeedback','menuHelp','groupBox_alle_aufgaben','groupBox_sage', 
'actionLoad','actionSave']
...
widgets_feedback=['menuSuche','menuNeue_Schularbeit','menuNeue_Aufgabe','menuHelp','comboBox_at_fb','groupBox_alle_aufgaben_fb','label_example', 'groupBox_fehlertyp','groupBox_feedback', 'groupBox_email', 'pushButton_send']
...
class Ui_Dialog_erstellen(object):
	...
	def setupUi(self, Dialog, dict_list_input_examples, beispieldaten_dateipfad, dict_titlepage, saved_file_path):
		...
		self.gridLayout_2.addWidget(self.label_sw_num_ges, 3, 0, 1, 1, QtCore.Qt.AlignLeft)
	...
	def retranslateUi(self, Dialog):
		...
		self.label_sw_num_ges_int.setText(_translate("Dialog", "%s"%str(self.data_gesamt['num_aufgabe'])))
		...
		self.label_sw_pkt_ges_int.setText(_translate("Dialog", "%s"%str(self.data_gesamt['gesamtpunkte']))) #+self.data_gesamt['ausgleichspunkte']
...
class Ui_MainWindow(object):
	...
	def open_subwindow_erstellen(self, dict_list_input_examples,beispieldaten_dateipfad, dict_titlepage, saved_file_path): #, dict_gesammeltedateien
		self.Dialog_erstellen = QtWidgets.QDialog(None, QtCore.Qt.WindowSystemMenuHint | QtCore.Qt.WindowTitleHint | QtCore.Qt.WindowCloseButtonHint)
		self.ui = Ui_Dialog_erstellen()
		self.ui.setupUi(self.Dialog_erstellen, dict_list_input_examples, beispieldaten_dateipfad, dict_titlepage, saved_file_path)	#, dict_gesammeltedateien
	...
	def setupUi(self, MainWindow):
		...
		for all in list_klassen:
			...
			exec('self.gridLayout_{0} = QtWidgets.QGridLayout(self.tab_{0})'.format(all))
			exec('self.gridLayout_{0}.setObjectName("gridLayout_{0}")'.format(all))			
			exec('self.scrollArea_{0} = QtWidgets.QScrollArea(self.tab_{0})'.format(all))
			...
			exec('self.scrollAreaWidgetContents_{}.setGeometry(QtCore.QRect(0, 0, 264, 235))'.format(all))
			exec('self.scrollAreaWidgetContents_{0}.setObjectName("scrollAreaWidgetContents_{0}")'.format(all))
			exec('self.verticalLayout_kapitel_{0} = QtWidgets.QVBoxLayout(self.scrollAreaWidgetContents_{0})'.format(all))
			...
			exec('self.scrollArea_{0}.setWidget(self.scrollAreaWidgetContents_{0})'.format(all))
			exec('self.gridLayout_{0}.addWidget(self.scrollArea_{0}, 5, 0, 1, 1)'.format(all))
			exec('self.tabWidget_klassen.addTab(self.tab_{0}, "{1}. Klasse")'.format(all, all[1]))


		self.groupBox_unterkapitel = QtWidgets.QGroupBox(self.centralwidget)
		self.groupBox_unterkapitel.setStyleSheet("background-color: rgb(217, 255, 215);")
		...
		self.actionNeue_Schularbeit_erstellen.setObjectName("actionNeue_Schularbeit_erstellen")
		...
		self.actionNeue_Schularbeit_erstellen.triggered.connect(self.neue_schularbeit_erstellen)
		...
		self.groupBox_grundkompetenzen_cr.setTitle(_translate("MainWindow", "Grundkompetenzen"))
		...
		self.groupBox_ausgew_gk_cr.setTitle(_translate("MainWindow", "Ausgewählte Grundkompetenzen"))
		...
		self.groupBox_bilder.setTitle(_translate("MainWindow", "Bilder (klicken, um Bilder zu entfernen)"))
		...
		self.groupBox_beispieleingabe.setTitle(_translate("MainWindow", "Beispieleingabe"))
		# self.groupBox_beispieleingabe.resize(500, 200)
		self.label.setText(_translate("MainWindow", "Info: Eingabe des Aufgabentextes zwischen \\begin{beispiel}...\\end{beispiel}"))
		...
		for all in list_klassen:
			...
			exec('self.gridLayout_cr_{0} = QtWidgets.QGridLayout(self.tab_cr_{0})'.format(all))
			exec('self.gridLayout_cr_{0}.setObjectName("gridLayout_cr_{0}")'.format(all))			
			exec('self.scrollArea_cr_{0} = QtWidgets.QScrollArea(self.tab_cr_{0})'.format(all))
			...
			exec('self.scrollAreaWidgetContents_cr_{} = QtWidgets.QWidget()'.format(all))
			exec('self.scrollAreaWidgetContents_cr_{}.setGeometry(QtCore.QRect(0, 0, 264, 235))'.format(all))
			exec('self.scrollAreaWidgetContents_cr_{0}.setObjectName("scrollAreaWidgetContents_cr_{0}")'.format(all))
			exec('self.verticalLayout_kapitel_cr_{0} = QtWidgets.QVBoxLayout(self.scrollAreaWidgetContents_cr_{0})'.format(all))
			...
			exec('self.combobox_kapitel_{} = QtWidgets.QComboBox(self.centralwidget)'.format(all))
			...
			for kapitel in dict_klasse_name:
				...
				combobox_kapitel.setItemText(i, _translate("MainWindow", dict_klasse_name[kapitel] + ' ('+ kapitel + ')'))
				combobox_kapitel.setMinimumHeight(25)
				combobox_kapitel.setStyleSheet("background-color: rgb(240, 240, 240);")
				i+=1

			
			spacerItem_unterkapitel = QtWidgets.QSpacerItem(20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
			verticalLayout_cr.addWidget(combobox_kapitel)
			combobox_kapitel.currentIndexChanged.connect(partial(self.comboBox_kapitel_changed_cr,verticalLayout_cr, combobox_kapitel, all,spacerItem_unterkapitel))
			...
			exec('self.scrollArea_cr_{0}.setWidget(self.scrollAreaWidgetContents_cr_{0})'.format(all))
			exec('self.gridLayout_cr_{0}.addWidget(self.scrollArea_cr_{0}, 5, 0, 1, 1)'.format(all))
													  
															   
																	
															 
													  
			exec('self.tab_widget_gk_cr.addTab(self.tab_cr_{0}, "{1}. Klasse")'.format(all, all[1]))
			...
			for unterkapitel in dict_klasse[first_element]:
				self.create_checkbox_unterkapitel(verticalLayout_cr, all, first_element, unterkapitel)
		...
		for all in dict_aufgabenformate:
			self.comboBox_af.addItem(_fromUtf8(""))
			self.comboBox_af.setItemText(i, _translate("MainWindow", dict_aufgabenformate[all]))
			i+=1

		self.gridLayout_7.addWidget(self.comboBox_af, 0, 0, 1, 1)
		self.gridLayout.addWidget(self.groupBox_aufgabenformat, 0, 2, 1, 1, QtCore.Qt.AlignRight)
		self.groupBox_aufgabenformat.setTitle(_translate("MainWindow", "Aufgabenformat"))
		...
		self.label_keine_auswahl.setText(_translate("MainWindow", "keine Auswahl nötig"))
		...
		self.groupBox_quelle.setTitle(_translate("MainWindow", "Quelle oder Autor (Vorname Nachname) - Eingabe: VorNac"))
		self.groupBox_quelle.hide()

		self.pushButton_save = QtWidgets.QPushButton(self.centralwidget)
		self.pushButton_save.setObjectName(_fromUtf8("pushButton_save"))
		self.gridLayout.addWidget(self.pushButton_save, 6, 2, 1, 1, QtCore.Qt.AlignRight) #
		...
		for all in list_klassen:
			...
			self.comboBox_klassen.setItemText(index, _translate("MainWindow", all[1]+'. Klasse'))
			index+=1

		self.comboBox_klassen.currentIndexChanged.connect(partial(self.comboBox_klassen_changed,'sage'))
		...
		self.comboBox_kapitel.currentIndexChanged.connect(partial(self.comboBox_kapitel_changed, 'sage'))
		...
		self.comboBox_unterkapitel.currentIndexChanged.connect(partial(self.comboBox_unterkapitel_changed,'sage'))
		...
		self.lineEdit_number.textChanged.connect(partial(self.adapt_choosing_list,'sage'))
		...
		self.comboBox_pruefungstyp.addItem("")
		self.comboBox_pruefungstyp.addItem("")
		self.comboBox_pruefungstyp.setItemText(0, _translate("MainWindow", "Schularbeit"))
		self.comboBox_pruefungstyp.setItemText(1, _translate("MainWindow", "Nachschularbeit"))
		self.comboBox_pruefungstyp.setItemText(2, _translate("MainWindow", "Wiederholungsschularbeit"))	
		self.comboBox_pruefungstyp.setItemText(3, _translate("MainWindow", "Wiederholungsprüfung"))
		self.comboBox_pruefungstyp.setItemText(4, _translate("MainWindow", "Grundkompetenzcheck"))
		self.comboBox_pruefungstyp.setItemText(5, _translate("MainWindow", "Übungsblatt"))
		...
		self.pushButton_titlepage.setText(_translate("MainWindow", "Titelblatt anpassen"))
		...
		self.groupBox_beurteilungsra.setTitle(_translate("MainWindow", "Beurteilungsraster"))
		...
		self.label_gesamtbeispiele.setText(_translate("MainWindow", "Anzahl der Aufgaben: 0 (Typ1: 0 / Typ2: 0)	 "))
		...
		self.comboBox_at_fb.setItemText(0, _translate("MainWindow", "Aufgaben Rückmeldung"))
																			 
		self.comboBox_at_fb.setItemText(1, _translate("MainWindow", "Allgemeine Rückmeldung"))
		...
		for all in list_klassen:
			...
			self.comboBox_klassen_fb.setItemText(i, _translate("MainWindow", all[1]+". Klasse"))
			i+=1

		self.comboBox_klassen_fb.currentIndexChanged.connect(partial(self.comboBox_klassen_changed,'feedback'))
		...
		self.comboBox_kapitel_fb.currentIndexChanged.connect(partial(self.comboBox_kapitel_changed, 'feedback'))
		...
		self.comboBox_unterkapitel_fb = QtWidgets.QComboBox(self.groupBox_alle_aufgaben_fb)
		self.comboBox_unterkapitel_fb.setObjectName("self.comboBox_unterkapitel_fb")

		self.comboBox_unterkapitel_fb.currentIndexChanged.connect(partial(self.comboBox_unterkapitel_changed, 'feedback'))
		...
		self.lineEdit_number_fb.textChanged.connect(partial(self.adapt_choosing_list,'feedback'))
		...
		self.comboBox_fehlertyp.setItemText(2, _translate("MainWindow", "Fehler in der Angabe"))
		self.comboBox_fehlertyp.setItemText(3, _translate("MainWindow", "Fehler in der Lösung"))
		self.comboBox_fehlertyp.setItemText(4, _translate("MainWindow", "Bild wird nicht (richtig) angezeigt"))
		self.comboBox_fehlertyp.setItemText(5, _translate("MainWindow", "Grafik ist unleserlich/fehlerhaft"))
		self.comboBox_fehlertyp.setItemText(6, _translate("MainWindow", "Beispiel ist doppelt vorhanden"))
		self.comboBox_fehlertyp.setItemText(7, _translate("MainWindow", "Falsche Kodierung (Grundkompetenz, Aufgabenformat, ...)"))
		...
		self.groupBox_feedback.setTitle(_translate("MainWindow", "Feedback bzw. Problembeschreibung"))
		...
		self.groupBox_email.setTitle(_translate("MainWindow", "Kontakt (E-Mail) für Nachfragen (optional)"))
	...
	def retranslateUi(self, MainWindow):
		_translate = QtCore.QCoreApplication.translate
		MainWindow.setWindowTitle(_translate("LaMA-Cria - LaTeX Mathematik Assistent Unterstufe", "LaMA-Cria - LaTeX Mathematik Assistent Unterstufe"))
		...
		try:
			...
			self.label_update.setText(_translate("MainWindow", 'Letztes Update: ' + self.modification_date(log_file).strftime('%d.%m.%y - %H:%M')))
		except FileNotFoundError:
			self.label_update.setText(_translate("MainWindow", "Letztes Update: ---"))
		#self.label_update.setText(_translate("MainWindow", "Letztes Update: ---"))
		self.combobox_searchtype.setItemText(0, _translate("MainWindow", "Alle Dateien ausgeben, die zumindest ein Suchkriterium enthalten"))

									 
																													   
		self.combobox_searchtype.setItemText(1, _translate("MainWindow", "Alle Dateien ausgeben, die alle Suchkriterien enthalten"))
		...
		self.actionFeedback.setText(_translate("MainWindow", "Feedback oder Fehler senden ..."))
		...
		self.actionBild_konvertieren_jpg_eps.setText(_translate("MainWindow", "Bild konvertieren (jpg zu eps)"))	
		...
		self.actionRefresh_Database.setText(_translate("MainWindow", "Refresh Database"))
		self.actionRefresh_Database.setShortcut(_translate("MainWindow", "F5"))
		self.actionNeue_Schularbeit_erstellen.setText(_translate("MainWindow", "Neue Schularbeit erstellen..."))
																								   
																				  
																		   
																 
																 
		self.actionNeue_Schularbeit_erstellen.setShortcut(_translate("MainWindow", "F2"))
																 
		self.actionNeue_Aufgabe_erstellen.setText(_translate("MainWindow", "Neue Aufgabe erstellen..."))
	...
	def check_for_update(self):
		...
		if update_check[0] != update_check[1]:
			...
			if ret==QtWidgets.QMessageBox.Yes:
				...
				filename_update=os.path.join(path_programm,'_database','_config','update','update_cria%s'%extension)
	...
	def show_info(self):
			...
			msg.setWindowTitle("Über LaMA-Cria - LaTeX Mathematik Assistent Unterstufe")
	...
	def reset_window(self):
		...
		for example in self.list_creator_topics[:]:
			cb_unterkapitel =  eval('self.cb_unterkapitel_{0}_{1}'.format(example[1], example[2]))
	...
	def create_kapitel(self, layout, klasse, kapitel):
		...
		exec('self.radioButton_k{0}_{1} = QtWidgets.QRadioButton(self.scrollAreaWidgetContents_k{0})'.format(klasse, kapitel)) 
		radioButton_klasse_kapitel=eval('self.radioButton_k{0}_{1}'.format(klasse, kapitel))
		...
		radioButton_klasse_kapitel.setText(_translate("MainWindow", dict_klasse_name[kapitel] + ' ('+ kapitel +')'))		
		radioButton_klasse_kapitel.toggled.connect(partial(self.chosen_radiobutton, klasse, kapitel))
	...
	def chosen_radiobutton(self, klasse, kapitel):
		dict_klasse_name =eval('dict_k{}_name'.format(klasse))
		self.groupBox_unterkapitel.setTitle(_translate("MainWindow", "Unterkapitel - "+str(klasse)+". Klasse - " +dict_klasse_name[kapitel]))
		...
		for all in dict_klasse[kapitel]:
			...
			exec('self.checkBox_k{0}_{1}_{2} = QtWidgets.QCheckBox(self.scrollAreaWidgetContents)'.format(klasse, kapitel, all))
			...
			checkBox.stateChanged.connect(partial(self.checkBox_checked, klasse, kapitel, all))
		...
		exec('self.btn_alle_{0}_{1} = QtWidgets.QPushButton(self.scrollAreaWidgetContents)'.format(klasse, kapitel))
	...
	def checkBox_checked(self, klasse, kapitel, unterkapitel):
		...
		checkBox=eval('self.checkBox_k{0}_{1}_{2}'.format(klasse, kapitel, unterkapitel))
	...
	def refresh_ddb(self):
		...
		for klasse in list_klassen: 
			for root, dirs, files in os.walk(os.path.join(path_programm,'_database', klasse)):
				for all in files:
	...
	#					file.close()
		...
		self.label_update.setText(_translate("MainWindow", 'Last Update: ' + self.modification_date(log_file).strftime('%d.%m.%y - %H:%M')))
		QtWidgets.QApplication.restoreOverrideCursor()
		msg.close()
	...
	def PrepareTeXforPDF(self):
		...
		filename_teildokument = os.path.join(path_programm,'Teildokument','Teildokument.tex')
		try:
		...
		if self.combobox_searchtype.currentText()=='Alle Dateien ausgeben, die zumindest ein Suchkriterium enthalten':
	...
	def create_checkbox_unterkapitel(self,layout, klasse, kapitel, unterkapitel):
		# exec('self.tab_cr_{} = QtWidgets.QWidget()'.format(all))
		# exec('self.tab_cr_{0}.setObjectName("tab_cr_{0}")'.format(all))
		exec('self.cb_unterkapitel_{0}_{1}= QtWidgets.QCheckBox(self.centralwidget)'.format(kapitel, unterkapitel))
		cb_unterkapitel =  eval('self.cb_unterkapitel_{0}_{1}'.format(kapitel, unterkapitel))
		#self.cb_thema.setChecked(True)
		cb_unterkapitel.setObjectName("cb_unterkapitel_{0}_{1}".format(kapitel, unterkapitel))
		layout.addWidget(cb_unterkapitel, QtCore.Qt.AlignTop)
		cb_unterkapitel.setText(_translate("MainWindow", dict_unterkapitel[unterkapitel]+ ' ('+unterkapitel+')' ))
		cb_unterkapitel.stateChanged.connect(partial(self.checkBox_checked_creator,cb_unterkapitel, klasse, kapitel, unterkapitel))
		aufgabe_chosen=[klasse, kapitel, unterkapitel]
		if aufgabe_chosen in self.list_creator_topics:
			cb_unterkapitel.setChecked(True)


	def comboBox_kapitel_changed_cr(self, verticalLayout_cr,combobox_kapitel, klasse, spacerItem_unterkapitel):
		#spacerItem = QtWidgets.QSpacerItem(20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
		...
		for unterkapitel in dict_klasse[chosen_kapitel]:
			self.create_checkbox_unterkapitel(verticalLayout_cr,klasse, chosen_kapitel, unterkapitel)
	...
	def add_picture(self):
		...
		list_filename = QtWidgets.QFileDialog.getOpenFileNames(None, 'Grafiken auswählen', os.path.dirname(self.saved_file_path[0]), 'Grafiken (*.eps)')
		if list_filename[0]==[]:
			return	
	...
	def convert_imagetoeps(self):
			...
			if ret==QtWidgets.QMessageBox.Yes:
				...
				filename = QtWidgets.QFileDialog.getOpenFileNames(None, 'Select a folder:', os.path.dirname(self.saved_file_path), 'Bilder (*.jpg; *.png)')
				if filename[0]!=[]:
					...
					for all in filename[0]:
						...
						if ext.lower()=='.jpg' or ext.lower()=='.jpeg':
							...
							self.warning_window('Die Datei konnte nicht konvertiert werden.')
							return
					msg = QtWidgets.QMessageBox()
					msg.setIcon(QtWidgets.QMessageBox.Information)
					msg.setWindowIcon(QtGui.QIcon(logo_path))
	...
	def save_file(self):
		...
		if self.creator_mode=='user':
			...
			'Bilder: {4}\n'.format(edit_titel, aufgabenformat, themen, self.lineEdit_quelle.text(), bilder))
			# msg.setInformativeText('Soll die PDF Datei erstellt werden?')
		...
		klasse_path_temp=os.path.join(path_programm,'_database',path_folder,'Einzelbeispiele')
		...
		if self.creator_mode=='admin':
			...
			if not os.path.exists(path_saved_files):
				...
				msg.setWindowIcon(QtGui.QIcon(logo_path))
				msg.setText('Der Ordner "Beispieleinreichung" konnte nicht gefunden werden und muss zuerst für Sie freigegeben werden.')
		...
		for all in dict_picture_path:
			...
			if x in textBox_Entry:
				...
				textBox_Entry=str(textBox_Entry).replace(tail,'../_database/Bilder/'+klasse+'_'+str(max_integer_file+1)+'_'+tail)
		...
		for all in list(dict_picture_path.values()):
			...
			if self.creator_mode=='admin':
				x=os.rename(copy_image_file_temp,'%s/_database/Bilder/'% path_programm +klasse+'_'+str(max_integer_file+1)+'_'+tail) ### direct save
				...
				x=os.rename(copy_image_file_temp,'%s/Beispieleinreichung/Bilder/'% path_programm +klasse+'_'+str(max_integer_file+1)+'_'+tail) ### indirect
		...
		if self.creator_mode=='admin':
			file_name=os.path.join(path_programm,'_database',klasse,'Einzelbeispiele',str(max_integer_file+1)+'.tex') ### direct save
			file=open(file_name,"w",encoding='utf8')
		else:
			file_name=os.path.join(path_programm,'Beispieleinreichung',klasse, str(max_integer_file+1)+'.tex') ### not direct save
		...
		chosen_af=list(dict_aufgabenformate.keys())[list(dict_aufgabenformate.values()).index(chosen_af_lang)]
		...
		file.write("\section{"+klasse.upper() +' - '+ themen_auswahl_joined +' - ' + edit_titel+" - "+ chosen_af.upper() +' - '+quelle+"}\n\n"
		...
		msg.setWindowIcon(QtGui.QIcon(logo_path))
		msg.setText('Die Aufgabe mit dem Titel\n\n"{0}"\n\nwurde gespeichert.'.format(edit_titel))
		msg.setDetailedText('Details\n'
		'Thema/Themen: {0}\n'
		'Punkte: {1}\n'
		'Klasse: {2}. Klasse\n'
		'Bilder: {3}'.format(themen_auswahl_joined, self.spinBox_punkte.value(), klasse[-1], images))
		msg.setStandardButtons(QtWidgets.QMessageBox.Ok)
		msg.exec_()
		self.reset_window()
	...
	def sage_load(self, external_file_loaded):
		...
		if external_file_loaded==False:
			...
			path_backup_file = QtWidgets.QFileDialog.getOpenFileName(None, 'Öffnen', os.path.dirname(self.saved_file_path), 'LaMA-Cria Datei (*.lamacria);; Alle Dateien (*.*)')
	...
	def sage_save(self, path_file): #path_file
		...
		# print(path_file)
		if path_file=='':
			path_backup_file = QtWidgets.QFileDialog.getSaveFileName(None, 'Speichern unter', self.saved_file_path, 'LaMA-Cria Datei (*.lamacria);; Alle Dateien (*.*)')
			if path_backup_file[0]=='':
				return
	...
	def comboBox_klassen_changed(self, list_mode):
		if list_mode=='sage':
			dict_klasse_name=eval('dict_{}_name'.format(list_klassen[self.comboBox_klassen.currentIndex()]))
			self.comboBox_kapitel.clear()
			self.comboBox_unterkapitel.clear()
			self.comboBox_kapitel.addItem("")
		if list_mode=='feedback':
			self.label_example.setText(_translate("MainWindow", "Ausgewählte Aufgabe: -"))
			dict_klasse_name=eval('dict_{}_name'.format(list_klassen[self.comboBox_klassen_fb.currentIndex()]))
			self.listWidget_fb.clear()
			self.comboBox_kapitel_fb.clear()
			self.comboBox_unterkapitel_fb.clear()
			self.comboBox_kapitel_fb.addItem("")
		...
		for all in dict_klasse_name.keys():
			...
			if list_mode=='feedback':
				self.comboBox_kapitel_fb.addItem(dict_klasse_name[all] + ' ('+all+')')
	...
	def comboBox_kapitel_changed(self, list_mode):
		dict_klasse = eval('dict_{}'.format(list_klassen[self.comboBox_klassen.currentIndex()]))

		if list_mode=='sage':
			dict_klasse = eval('dict_{}'.format(list_klassen[self.comboBox_klassen.currentIndex()]))
			self.comboBox_unterkapitel.clear()		

			kapitel_shortcut = list(dict_klasse.keys())[self.comboBox_kapitel.currentIndex()-1]
		...
		if list_mode=='feedback':
			dict_klasse = eval('dict_{}'.format(list_klassen[self.comboBox_klassen_fb.currentIndex()]))
			self.comboBox_unterkapitel_fb.clear()		

			kapitel_shortcut = list(dict_klasse.keys())[self.comboBox_kapitel_fb.currentIndex()-1]
			self.comboBox_unterkapitel_fb.addItem("")		

		index=1
		for all in dict_klasse[kapitel_shortcut]:
			if list_mode=='sage':
				self.comboBox_unterkapitel.addItem(dict_unterkapitel[all] + ' ('+all+')')
				# self.comboBox_unterkapitel.setItemText(index, _translate("MainWindow", dict_unterkapitel[all] + ' ('+all+')'))
			if list_mode=='feedback':
				self.comboBox_unterkapitel_fb.addItem(dict_unterkapitel[all] + ' ('+all+')')
	...
	def adapt_choosing_list(self, list_mode):
		...
		if list_mode=='sage':
			...
			if self.comboBox_kapitel.currentText() is not '':
				kapitel_shortcut=list(dict_klasse_name.keys())[self.comboBox_kapitel.currentIndex()-1]
			else:
				kapitel_shortcut=''

			if self.comboBox_unterkapitel.currentText() is not '':
				shortcut=re.findall(r"\((.*)\)",self.comboBox_unterkapitel.currentText())
				...
				if str(list_klassen[self.comboBox_klassen.currentIndex()]) in file_path:
					if kapitel_shortcut=='':
						...
						if unterkapitel_shortcut=='':
							...
							thema_shortcut=kapitel_shortcut + '.' + unterkapitel_shortcut
		...
		if list_mode=='feedback':
			...
			if self.comboBox_kapitel_fb.currentText() is not '':
				kapitel_shortcut=list(dict_klasse_name.keys())[self.comboBox_kapitel_fb.currentIndex()-1]
			else:
				kapitel_shortcut=''

			if self.comboBox_unterkapitel_fb.currentText() is not '':
				shortcut=re.findall(r"\((.*)\)",self.comboBox_unterkapitel_fb.currentText())
				unterkapitel_shortcut=shortcut[-1]
			else:
				unterkapitel_shortcut=''	
			...
			for all in beispieldaten_dateipfad.keys():
				file_path=beispieldaten_dateipfad[all]
				if str(list_klassen[self.comboBox_klassen_fb.currentIndex()]) in file_path:
					if kapitel_shortcut=='':
						add_filename_to_list(file_path)
					else:
						if unterkapitel_shortcut=='':
	...
	def save_dict_examples_data(self):
		...
		"Klasse": self.lineEdit_klasse.text(), "Notenschluessel": [self.spinBox_2.value(),self.spinBox_3.value(),self.spinBox_4.value(),self.spinBox_5.value()],
		'num_aufgabe': num_aufgaben,'gesamtpunkte': gesamtpunkte, 'copy_images' : self.list_copy_images}
	...
	def nummer_clicked_fb(self, item):
		self.label_example.setText(_translate("MainWindow", "Ausgewählte Aufgabe: {0} ({1})".format(item.text(),self.comboBox_klassen_fb.currentText())))
	...
	def sage_aufgabe_add(self,aufgabe):
		...
		self.label_gesamtbeispiele.setText(_translate("MainWindow", "Anzahl der Aufgaben: {0}".format(num_total)))	
	...
	def punkte_changed(self, bsp_string):
		...
		self.label_sg_pkt.setText(_translate("MainWindow", "% (ab {})".format(list_punkte[0])))
		self.label_g_pkt.setText(_translate("MainWindow",  "% (ab {})".format(list_punkte[1])))
		self.label_b_pkt.setText(_translate("MainWindow",  "% (ab {})".format(list_punkte[2])))
		self.label_g_pkt_2.setText(_translate("MainWindow",	 "% (ab {})".format(list_punkte[3])))
		#self.label_ng_pkt.setText(_translate("MainWindow",	 "% (<{})".format(pkt_ge),None))

		self.label_gesamtpunkte.setText(_translate("MainWindow", "Gesamtpunkte: %i"%gesamtpunkte))
	...
	def move_button_pressed(self, direction, bsp_string):
		...
		if direction=='up':
			a,b=list_sage_examples.index(bsp_string), list_sage_examples.index(bsp_string)-1
		if direction=='down':
			a,b=list_sage_examples.index(bsp_string), list_sage_examples.index(bsp_string)+1
	...
	def sage_aufgabe_create(self, file_loaded=False):
		...
		for bsp_string in list_sage_examples:
			...
			x.setTitle(_translate("MainWindow", "{0}. Aufgabe".format(str(num_of_example))))
			...
			label_aufgabe.setText(_translate("MainWindow", "{0}. Klasse - {1}".format(klasse[1], example)))
			...
			label_title.setText(_translate("MainWindow", 'Titel: {}'.format(list_input[2]))) #list_titles[i-1]
			...
			self.pushButton_up.clicked.connect(partial(self.move_button_pressed, 'up', bsp_string))	
			...
			self.pushButton_down.clicked.connect(partial(self.move_button_pressed, 'down', bsp_string))
			...
			self.pushButton_delete.clicked.connect(partial(self.btn_delete_pressed, bsp_string))
	...
	def pushButton_vorschau_pressed(self, ausgabetyp, index, maximum):
			...
			for all in self.beispieldaten_dateipfad:
				if klasse.upper() in all:
					if name==os.path.basename(self.beispieldaten_dateipfad[all]):
						dict_gesammeltedateien[bsp_string]=self.beispieldaten_dateipfad[all]
		...
		for bsp_string in list_sage_examples:
			...
			if beispiel_typ=='beispiel':
				...
				vorschau.write("\\begin{beispiel}{"+str(spinBox_pkt)+"}\n"+divided_example[1]+"\n"+divided_example[2]+"\n\n")
			...
			if beispiel_typ=='langesbeispiel':
				vorschau.write("\\begin{langesbeispiel} \item["+str(spinBox_pkt)+"]\n"+divided_example[1]+"\n"+divided_example[2]+"\n\n")
	...
	def pushButton_erstellen_pressed(self):	
		...
		try:
			...
			self.saved_file_path=path_programm
		self.open_subwindow_erstellen(self.dict_list_input_examples, self.beispieldaten_dateipfad, self.dict_titlepage, self.saved_file_path)
